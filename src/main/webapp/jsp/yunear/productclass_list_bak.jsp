	...
	<div class="page-content">
		<div class="row">
			<div class="col-xs-12">
				...
				<!-- 查询  -->
				<form class="form-horizontal" role="form" action="productclass/list.do" method="post" name="Form" id="Form">
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										<td class="center" style="width:30px;">
											<label class="pos-rel">
												<input type='checkbox' class="ace" name='ids' value="${var.PRODUCTCLASS_ID}" />
												<span class="lbl"></span>
											</label>
										</td>
										<td class="center" style="width:50px;">${vs.index+1}</td>
										<td>${var.CLASS_NAME}</td>
										<td>${var.CLASS_PARENTID}</td>
										<td>${var.GRADE}</td>
										...
										<td class="center" style="width:120px;">
											...
											<div class="hidden-sm hidden-xs btn-group">
												...
												<c:if test="${QX.edit == 1 }">
													<a title="编辑" onclick="edit('${var.PRODUCTCLASS_ID}');" class="btn btn-mini btn-success">
														<i class="ace-icon fa fa-edit"></i>
													</a>
												</c:if>
												<c:if test="${QX.del == 1 }">
													<a title="删除" onclick="del('${var.PRODUCTCLASS_ID}');" class="btn btn-mini btn-danger">
													...
													<c:otherwise>
														<div class="inline pos-rel">
															...
															<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																<c:if test="${QX.edit == 1 }">
																	<li>
																		<a href="javascript:edit('${var.PRODUCTCLASS_ID}');" class="tooltip-success" data-rel="tooltip" title="编辑">
																			<span class="green">
																...
																<c:if test="${QX.del == 1 }">
																	<li>
																		<a href="javascript:del('${var.PRODUCTCLASS_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																			<span class="red">
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>/productclass/excel.do';
		}
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>/productclass/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>/productclass/delete.do?PRODUCTCLASS_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>/productclass/goEdit.do?PRODUCTCLASS_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							$.ajax({
								type: "POST",
								url: '<%=basePath%>/productclass/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
