...
public class Page {
	...
	public Page() {
		try {
			this.showCount = Integer.parseInt(SysUtil.SYSTEM_DEFAULT_PAGE);
		} catch (Exception e) {
			this.showCount = 15;
		}
	}
	...
	public String getPageStr() {
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				...
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:55px;height:32px;text-align:center;float:left\" placeholder=\"页码\"/></li>\n");
			...
			sb.append("	<li><select class=\"form-control\" title='显示条数' style=\"width:55px;height:32px;float:left;\" onchange=\"changeCount(this.value)\">\n");
